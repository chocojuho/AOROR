...
app.get('/index', function(req, res) {
    db.collection("documents").find().sort({ _id: 1 }).toArray(function(err, result) {
        res.render("main.ejs", { documents: result });
    })
})
...
app.post('/add', function(req, res) {
    db.collection('documents').findOne({ _id: req.body.title }, function(err, result) {
        if (result == null) {
            db.collection('documents').insertOne({ _id: req.body.title, tearbig: req.body.tearbig, tearsmall: req.body.tearsmall, answer: req.body.answer, testcase: req.body.testcase }, function(err, result) {
...
app.get('/find/:id', function(req, res) {
    db.collection("documents").findOne({ _id: req.params.id }, function(err, result) {
        console.log(result);
        res.render("find.ejs", { documents: result });
    });
})
